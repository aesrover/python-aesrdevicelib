# Source https://github.com/ghirlekar/bno055-python-i2c/blob/master/BNO055.py
...
class BNO055(i2c_device.I2cDevice):
    ...
    def __init__(self, i2cAddress= _DEFAULT_I2C_ADDRESS, mode = None, *args, **kwargs):
        ...
        if self.readBytes(BNO055.BNO055_CHIP_ID_ADDR)[0] != BNO055.BNO055_ID:
            ...
            if self.readBytes(BNO055.BNO055_CHIP_ID_ADDR)[0] != BNO055.BNO055_ID:
                raise IOError("BNO055 device failed to startup")
    ...
    def setMode(self, mode):
        ...
        self.writeBytes(BNO055.BNO055_OPR_MODE_ADDR, [self.deviceMode])
        time.sleep(0.03)

    def setExternalCrystalUse(self, useExternalCrystal = True):
        prevMode = self.deviceMode
        self.setMode(BNO055.OPERATION_MODE_CONFIG)
        time.sleep(0.025)
        self.writeBytes(BNO055.BNO055_PAGE_ID_ADDR, [0])
